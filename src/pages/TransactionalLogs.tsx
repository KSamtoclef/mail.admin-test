...
import { fetchTransactionalLogs, type ActivityFilters } from '@/lib/services';
import type { TransactionalLog } from '@/types/database';

const PAGE_SIZE = 20;

export function TransactionalLogs() {
  const [data, setData] = useState<TransactionalLog[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const result = await fetchTransactionalLogs(filters);
      setData(result.data);
      setTotal(result.total);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to load transactional logs');
    } finally {
      setLoading(false);
    }
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="card overflow-hidden">
        {loading ? (
          ...
          <EmptyState icon={<FileText className="h-6 w-6" />} title="No transactional emails" description="Transactional emails from connected systems will appear here." />
        ) : (
          <>
                ...
                <tbody>
                  {data.map((row) => (
                    <tr key={row.id} className="border-b border-neutral-100 table-row-hover">
                      ...
                      <td className="px-4 py-3 text-neutral-600 capitalize">{row.email_type.replace(/_/g, ' ')}</td>
                      <td className="px-4 py-3 text-neutral-600 max-w-xs truncate">{row.subject || '—'}</td>
