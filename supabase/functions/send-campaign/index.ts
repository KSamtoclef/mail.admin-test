...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!campaign.sender_email || !EMAIL_REGEX.test(campaign.sender_email.trim())) {
      return jsonResponse({ error: "Campaign sender email is missing or invalid." }, 400);
    }
    ...
    if (campaign.audience_type === "d1_contacts") {
      const requestedCount = contactCount && Number.isFinite(contactCount) && contactCount > 0
        ? Math.floor(contactCount)
        : (campaign.d1_contact_count && campaign.d1_contact_count > 0 ? campaign.d1_contact_count : null);
    ...
    const eligibleContacts = allContacts.filter((c) => !suppressedSet.has(c.email.toLowerCase()));
    ...
    for (let i = 0; i < eligibleContacts.length; i += batchSize) {
      ...
      const { data: inserted, error: insertError } = await supabase
        ...
        .insert(batch.map((c) => ({ campaign_id: campaignId, email: c.email, status: "pending" })))
        .select("id, email");
      if (insertError) throw new Error(insertError.message);
      if (!inserted) continue;

      const contactMap = new Map(batch.map((c) => [c.email.toLowerCase(), c]));
      ...
      for (const recipient of inserted) {
        try {
          ...
          if (contact) {
            if (contact.full_name) { personalization.FullName = contact.full_name; htmlContent = htmlContent.replaceAll("[[FullName]]", contact.full_name); }
            if (contact.username) { personalization.Username = contact.username; htmlContent = htmlContent.replaceAll("[[Username]]", contact.username); }
            if (contact.country) { personalization.Country = contact.country; htmlContent = htmlContent.replaceAll("[[Country]]", contact.country); }
          ...
          if (sendResult.success) {
            ...
            await supabase.from("campaign_recipients").update({ status: "failed", error_info: sendResult.error }).eq("id", recipient.id);
            await supabase.from("email_activity").insert({
              recipient_email: recipient.email,
              campaign_name: campaign.name,
              email_type: "broadcast",
          ...
          await supabase.from("campaign_recipients").update({ status: "failed", error_info: errorMsg }).eq("id", recipient.id);
          failedCount++;
        }
      }
    ...
    return jsonResponse({ success: true, sent: sentCount, failed: failedCount, total: eligibleContacts.length, suppressed: suppressedCount });
  } catch (err) {
...
async function fetchD1Contacts(count: number | null): Promise<RecipientContact[]> {
  if (!D1_WORKER_ENDPOINT || !D1_WORKER_AUTH_TOKEN) throw new Error("D1 Worker secrets are not configured.");
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), 60000);
  try {
    const response = await fetch(`${D1_WORKER_ENDPOINT}/contacts/retrieve`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${D1_WORKER_AUTH_TOKEN}` },
      body: JSON.stringify({ count: count || 50000 }),
      signal: controller.signal,
    });
    if (!response.ok) throw new Error(`D1 Worker returned HTTP ${response.status}: ${await response.text().catch(() => "")}`);
    const data = await response.json();
    return (data.contacts || []) as RecipientContact[];
  } finally {
...
async function sendViaResend(params: { to: string; subject: string; html: string; from: string; replyTo?: string; }): Promise<{ success: boolean; messageId?: string; error?: string }> {
  ...
  try {
    const body: Record<string, string> = { to: params.to, subject: params.subject, html: params.html, from: params.from };
    ...
    const data = await response.json();
    if (!response.ok) return { success: false, error: data.message || `Resend API error: ${response.status}` };
