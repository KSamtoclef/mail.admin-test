...
1. New Tables
- `recipients` — the audience email list driving audience selection and segment filters.
  Stores email, full_name, country, status, and metadata. The Cloudflare Worker resolves
  verified personalization at send time, but recipient metadata here powers audience selection.

2. Security
- RLS enabled. All policies gated by admin_profiles existence, scoped TO authenticated.
*/
...
CREATE POLICY "admin_select_recipients" ON public.recipients
...
CREATE POLICY "admin_insert_recipients" ON public.recipients
...
CREATE POLICY "admin_update_recipients" ON public.recipients
  FOR UPDATE TO authenticated
  USING (EXISTS (SELECT 1 FROM public.admin_profiles ap WHERE ap.user_id = auth.uid()))
  WITH CHECK (EXISTS (SELECT 1 FROM public.admin_profiles ap WHERE ap.user_id = auth.uid()));

DROP POLICY IF EXISTS "admin_delete_recipients" ON public.recipients;
CREATE POLICY "admin_delete_recipients" ON public.recipients
